...
export async function generateBackgroundImage(
  ...
  const basePrompt =
    settings?.prompt ||
    `Нарисуй горизонтальное атмосферное изображение: интерьер жуткого многоквартирного многоэтажного жилого дома ночью (советская многоэтажка, хоть и старинная и пошарпанная — это именно жилой дом, не замок и не лес). Вид изнутри подъезда или квартиры. Тёмные коридоры, облупленные стены, мерцающий свет, почтовые ящики, старые двери. Стиль: {style}. Без людей, без текста, без надписей. Горизонтальная ориентация. Лор мира: ${lore.substring(0, 150)}. Высокое качество, детализация. ОБЯЗАТЕЛЬНО верни прямую ссылку на сгенерированное изображение.`;
  const prompt = applyMacros(basePrompt, {
    style,
    stage: String(stage),
...
export async function generateBossImage(
  ...
  const basePrompt =
    settings?.prompt ||
    `Нарисуй горизонтальное изображение огромного ужасающего босса для славянской игры ужасов. Стиль: {style}. Уровень босса: {boss_level}. Фон: ${bgPromptHint}. Эпичный, детализированный, горизонтальная ориентация. Без текста, без надписей. Высокое качество. ОБЯЗАТЕЛЬНО верни прямую ссылку на сгенерированное изображение.`;
  const prompt = applyMacros(basePrompt, {
    style,
    stage: String(stage),
